#if defined(CONFLUENCE_DOL_YAML)

#include "confluence/yaml.h"
#include "confluence/log.h"
#include <yaml.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static ConfluenceYamlNode* new_node(ConfluenceYamlKind kind) {
    ConfluenceYamlNode* n = (ConfluenceYamlNode*)calloc(1, sizeof(ConfluenceYamlNode));
    if (n) n->kind = kind;
    return n;
}
...
void confluence_yaml_free(ConfluenceYamlNode* node) {
    while (node) {
        ConfluenceYamlNode* next = node->next;
        confluence_yaml_free(node->children);
        free(node->key);
        free(node->value);
        free(node);
        node = next;
...
static void append(ConfluenceYamlNode* parent, ConfluenceYamlNode* child) {
    if (!parent->children) { parent->children = child; return; }
    ConfluenceYamlNode* tail = parent->children;
    while (tail->next) tail = tail->next;
    tail->next = child;
}

static int build_body(yaml_parser_t* p, ConfluenceYamlNode* into);

// Consume the content represented by `ev` and attach it to `parent` under
// optional `key`. Takes ownership of `key` and `ev`.
static int consume_event(yaml_parser_t* p, ConfluenceYamlNode* parent, char* key, yaml_event_t* ev) {
    int rc = 1;
    switch (ev->type) {
        case YAML_SCALAR_EVENT: {
            ConfluenceYamlNode* n = new_node(CONFLUENCE_YAML_SCALAR);
        ...
        case YAML_SEQUENCE_START_EVENT: {
            ConfluenceYamlNode* n = new_node(CONFLUENCE_YAML_SEQUENCE);
            if (!n) { free(key); rc = 0; break; }
            n->key = key;
            append(parent, n);
            rc = build_body(p, n);
        ...
        case YAML_MAPPING_START_EVENT: {
            ConfluenceYamlNode* n = new_node(CONFLUENCE_YAML_MAPPING);
            if (!n) { free(key); rc = 0; break; }
            n->key = key;
            append(parent, n);
            rc = build_body(p, n);
...
static int build_body(yaml_parser_t* p, ConfluenceYamlNode* into) {
    int is_map = (into->kind == CONFLUENCE_YAML_MAPPING);
...
ConfluenceYamlNode* confluence_yaml_parse_file(const char* path) {
    ...
    ConfluenceYamlNode* root = new_node(CONFLUENCE_YAML_MAPPING);
    if (!root) {
    ...
    for (;;) {
        ...
        if (type == YAML_MAPPING_START_EVENT) {
            ...
            root->kind = CONFLUENCE_YAML_SEQUENCE;
            yaml_event_delete(&ev);
            if (!build_body(&parser, root)) { ok = 0; break; }
        } else if (type == YAML_SCALAR_EVENT) {
            root->kind = CONFLUENCE_YAML_SCALAR;
    ...
    if (!ok) {
        LOG_ERROR("yaml parse error at line %lu col %lu: %s",
            (unsigned long)parser.problem_mark.line + 1,
            (unsigned long)parser.problem_mark.column + 1,
            parser.problem ? parser.problem : "(unknown)");
        confluence_yaml_free(root);
        root = NULL;
    }
...
const ConfluenceYamlNode* confluence_yaml_find(const ConfluenceYamlNode* parent, const char* key) {
    ...
    for (const ConfluenceYamlNode* c = parent->children; c; c = c->next) {
...
const char* confluence_yaml_get_str(const ConfluenceYamlNode* parent, const char* key) {
    const ConfluenceYamlNode* n = confluence_yaml_find(parent, key);
    return (n && n->kind == CONFLUENCE_YAML_SCALAR) ? n->value : NULL;
}

int confluence_yaml_seq_count(const ConfluenceYamlNode* node) {
    if (!node) return 0;
    int n = 0;
    for (const ConfluenceYamlNode* c = node->children; c; c = c->next) n++;
    return n;
}

#endif /* CONFLUENCE_DOL_YAML */
